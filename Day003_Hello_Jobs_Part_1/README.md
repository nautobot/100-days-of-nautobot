...
In order for the job to be available, we will need to tell Nautobot by registering it. Remember the ```invoke debug``` command that conveniently builds and starts all the components for us? Turns out there is an ```invoke``` command for ```post_upgrade``` as well.
