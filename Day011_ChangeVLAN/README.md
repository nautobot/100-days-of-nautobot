...
In Day 10's challenge, we were able to use Nautobot Jobs to perform 'show' commands on the devices Nautobot knows about. In today's challenge, we will move one step further by performing commands that make operational changes, namely changing vlan on a port. 

Before we get started, let's prepare the lab environment as we are doing it a bit different in today's lab. 
...
We will use the special global variable ```name``` to indicate the name of the Job: 
...
3. Only have Ethernet as the option to allow for VLAN change (we do not want to change vlan on, say, loopback interfaces). 
...
Notice we used ```"name__ic": "Ethernet"``` with double underscore to indicate name "includes" keyword of "Ethernet". 

Then, we can define ```IntegerVar``` to enter the VLAN number as well as the Meta class for meta information: 
...
```python
    def run(self, device_location, device, interface, vlan):
        ...
        self.logger.info(
            f"Successfully added VLAN {vlan} to {interface.name} on {device.name}!"
        )
```
...
This is great--we can now change the VLAN on a port with Nautobot! 

Let's make one more improvement. Instead of just using ```IntegerVar``` to indicate the VLAN number, we should scope the job to only existing VLANs. 
...
class ChangeVLAN_by_Function(Job):
    ...
    def run(self, device_location, device, interface, vlan):
        ...
        # If an excpetion is not raise the configuration was implemented successfully
        self.logger.info(
            f"Successfully added {vlan.name} VLAN to {interface.name} on {device.name}!"
        )
...
Now when we go to the job, only the existing VLANs will show up as the options: 
...
class ChangeVLAN(Job):
    ...
    def run(self, device_location, device, interface, vlan):
        ...
        # If an exception is not raised the configuration was implemented successfully
        self.logger.info(
            f"Successfully added VLAN {vlan} to {interface.name} on {device.name}!"
        )
...
Can you think of another improvement that we can make to the VLAN change job? Go ahead and post your thoughts on a social media of your choice, make sure you use the tags `#100DaysOfNautobot` `#JobsToBeDone` and tag `@networktocode`, so we can share your progress! 
